...
class RegisterViewModel  @Inject constructor(private val notesRepository: NotesRepository, private val sharedPreferences: SharedPreferences, private val json: Json) : ViewModel() {
    ...
    fun register(name: String, mobile: String,email: String, password: String, lifecycleOwner: LifecycleOwner) {
        notesRepository.register(RegisterRequest(name, mobile, email, password)).observe(lifecycleOwner) {
            when(it.status){
